...
-- 重點：資材班→改名資材課；資材→併入資材課（停用「資材」，其區域資材課已涵蓋）。
-- 作法：對照表內的班別，先停用「不在目標清單」的舊區域（刪除/合併），
--       再插入/重新啟用目標區域並更新排序。文字快照存於 defects.area_name，歷史不受影響。
-- 「空白：可自行填寫」為操作提示（App 已內建自行填寫欄），不建為區域。

-- (a) 資材班 → 資材課（保留原有區域與歷史關聯）；若已存在資材課則略過改名
update responsible_units set name = '資材課'
where name = '資材班'
  and not exists (select 1 from responsible_units where name = '資材課');

-- (b) 資材 併入資材課：停用「資材」班別（區域資材課已涵蓋，歷史缺失關聯保留、不刪）
update responsible_units set is_active = false where name = '資材';
...
insert into responsible_units (name, sort_order, is_active) values
  ...
  ('品保課', 7, true), ('工務課', 8, true), ('資材課', 9, true),
  ('其它', 10, true), ('外包業務', 11, true), ('配料室', 12, true)
